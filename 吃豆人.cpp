...
//
//    // 否则根据目标位置和当前位置的关系选择最佳移动方向
//    if (tx < person_x && a[person_x - 1][person_y] != 4) return 'U';
//    if (tx > person_x && a[person_x + 1][person_y] != 4) return 'D';
//    if (ty < person_y && a[person_x][person_y - 1] != 4) return 'L';
//    if (ty > person_y && a[person_x][person_y + 1] != 4) return 'R';
//
//    // 如果无法移动到目标位置，返回停留
//    return 'S';
//}
